...
class WarehouseDateProcessing():
    ...
    def getGenFilesNameList(self, directory, extension):
        for (dirpath, dirnames, filenames) in walk(directory):
            return (f for f in filenames if f.endswith('.' + extension))

    def CalculateDistance3240(self):
        d_ij = {}
        path_list = []
        current_path = os.getcwd()
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/-1/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/0/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/1/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/2/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/3/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/4/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/5/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/6/csv")
        path_list.append(current_path + r"/data_new/distances/3240_pods_distances_data/7/csv")
        print(path_list)

        str_gen_name_list = []
        # read distance
        print("start read distance files:")
        for files_path in path_list:
            gen_name_list = self.getGenFilesNameList(directory = files_path , extension = "txt")
            print("read files_path:", files_path)

            for name in gen_name_list:
                #print(idx, ": ", name)
                file_name = files_path + "/" + str(name)
                ...
                for line in Lines: 
                    str_text = line.strip()
                    data_list = str_text.split(';')
                    d_ij[data_list[0], data_list[1]] = float(data_list[2])
        print("finish load all distance data:", len(str_gen_name_list))
        return d_ij
...
class Demo():
    ...
    def initData(self):
        ...
        #d_ij = warehouse_data_processing.CalculateDistance3240()
    ...
    def local_search_shelves(self, batch: BatchNew):
        ...
        if len(batch.route) == 0:
            ...
            while len(assigned) != len(batch.items):
                ...
                #print("##pd start: def local_search_shelves(self, batch: BatchNew):")
                #starttime_calc = time.time()
                ...
                #t_calc = time.time() - starttime_calc
                #print("##pd end: def local_search_shelves(self, batch: BatchNew):", t_calc)
...
class VariableNeighborhoodSearch(IteratedLocalSearchMixed):
    ...
    def reduced_vns(self, max_iters, t_max, k_max, no_improve_steps):
        ...
        while iters < max_iters and t < t_max and iter_flag < no_improve_steps:
            ...
            while k <= k_max:
                ...
                self.local_search()
                t_calc = time.time() - starttime_calc
                print("## end local_search", t_calc)

                if len(self.batches) > 1:
                    #print("##start randomized_local_search")
                    self.randomized_local_search(max_iters=5, k=k)
                    #print("##end randomized_local_search:")
                else:
                    pass
                neighbor_fit = self.get_fitness_of_solution()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-dedicated', dest='dedicated',
                       action='store_true',
                       help='dedicated shevels', default="")
    parser.add_argument('-mixed_shevels')
    ...
    NUM_ORDERSS = [int(args.num_orderss)]  #100, 200, 300
    MEANS = [str(args.means)] #"1x6", "5"
    ITER_NUM = 1000
    NO_IMPROVE_STEPS_LIST = [100]
    #K_MAX_LIST = [int(args.step)] # 3, 4
    K_MAX_LIST = [4] # 3, 4
            ...
            for NUM_ORDERS in NUM_ORDERSS:
                for MEAN in MEANS:
                    ...
                    if args.dedicated:
                        ...
                        if args.mixed_shevels == "5":
                            storagePolicies['mixed'] = 'data_new/sku{}/pods_items_mixed_shevels_1-5.txt'.format(SKU)
                        elif args.mixed_shevels == "10":
                            storagePolicies['mixed'] = 'data_new/sku{}/pods_items_mixed_shevels_1-10.txt'.format(SKU)
                        elif args.mixed_shevels == "20":
                            storagePolicies['mixed'] = 'data_new/sku{}/pods_items_mixed_shevels_1-20.txt'.format(SKU)
                        print(storagePolicies['mixed'])
                        print("##mixed!")                  
                        ...
                        for NO_IMPROVE_STEPS_ITEM in NO_IMPROVE_STEPS_LIST:
                            for K_MAX_ITEM in K_MAX_LIST:
                                ...
                                if args.dedicated:
                                    ...
                                    vns.write_solution_to_xml(
                                        'solutions/{}/orders_{}_mean_{}_sku_{}{}_{}_no_improve_{}_kmax_{}_m{}.xml'.format(str(args.output), str(NUM_ORDERS), MEAN, SKU,
                                                                                             SUBSCRIPT, STORAGE_STRATEGY, NO_IMPROVE_STEPS_ITEM, str(K_MAX_ITEM), str(args.mixed_shevels))
                                    )
                                    sols_and_runtimes[runtime] = (vns.get_fitness_of_solution(), {batch.ID: batch.route for
                                                                   batch in vns.batches.values()})

                                    pickle_file_path = "solutions/" + str(args.output) + "/orders_" + str(NUM_ORDERS) + "_mean_" + str(MEAN) + "_sku_" + str(SKU) + str(SUBSCRIPT) + "_" \
                                        + str(STORAGE_STRATEGY) + "_no_improve_" + str(NO_IMPROVE_STEPS_ITEM) + "_kmax_" + str(K_MAX_ITEM) + "_m" + str(args.mixed_shevels) + "_result" + ".txt"
